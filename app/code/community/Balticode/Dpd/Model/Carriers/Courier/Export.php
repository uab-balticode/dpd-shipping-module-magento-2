...
class Balticode_Dpd_Model_Carriers_Courier_Export
    extends Mage_Core_Model_Config_Data
{
     ...
     * @return
     */
    public function run($scope_id = null)
    {
        $this->collectData($scope_id); //Collecting data to export
    ...
    public function collectData($scope_id = null)
    {
        if ($scope_id === null) {
            $scope_id = Mage::helper('dpd/data')->getStoreId();
        }

        $this->fileName = Mage::getSingleton('dpd/carriers_courier_courier')->key
            .'.'
            .$this->fileExtends;
        ...
        $this->fileContent = Mage::getModel('dpd/deliveryprice')->collectRestriction(
            false,
            Mage::getSingleton('dpd/carriers_courier_courier')->key,
            $scope_id);
    }
